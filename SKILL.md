...
① 分析活动列表（DOM级提取）→ 条件筛选(≥6折/≤20天/非爆款秒杀/非独立日/不过期/同名去重/折扣降序排序/日期连续/最多6个)
→ ② 打开 Drawer → ③ 勾选专题活动
...
9. **活动筛选（v4.1.2）**：
   ...
   - 弹窗日期全角括号 `（N天）` 已兼容
10. **天数计算（v4.1.3）**：
    - `days_between` 不加 `+1`，与 Temu UI 显示一致
    - 例：06-30~07-20 = 20天，不是21天
11. **同日期按折扣排序（v4.1.3）**：
    - `sort(key=lambda x: (x["start"], -x["discount"]))`
    - 同一天开始的活动，高折扣优先入选
12. **排除特定活动（v4.1.3）**：
    - `EXCLUDE_KEYWORDS` 添加活动名关键词即可排除
    - 当前排除：爆款、秒杀、独立日
13. **回滚命令**：
    - `git tag` 查看版本
    - `git checkout v4.1.3` 回滚到此版本（当前稳定版）
    - `git checkout v4.1.2` 或 `v4.1.1` 回滚到早期版本
